...
class ConfigOverviewController extends ControllerBase {
  ...
  public function content() {
    ...
    if ($this->moduleHandler->moduleExists('config_split')) {
      ...
      $splits = $this->configManager->findConfigEntityDependenciesAsEntities('module', ['config_split']);
    ...
    foreach ($configs as $config) {
      ...
      $row = [
        ...
        $this->renderer->render($overriddenImage),
        $this->renderer->render($overrideRows),
        $this->renderer->render($excludedImage),
      ];
      if ($this->moduleHandler->moduleExists('config_split')) {
        $row[] = $this->renderer->render($protectedOnImportImage);
      }
      $rows[] = $row;
    }
    $table = [
